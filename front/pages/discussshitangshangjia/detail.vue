...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshitangshangjiaCleanType')
			if(cleanType){
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussshitangshangjia', this.id);
		...
		methods: {
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussshitangshangjia', this.id);
				let reg=new RegExp('http://localhost:8080/springbootyx06s4r4/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discussshitangshangjia',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussshitangshangjia', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussshitangshangjia',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscussshitangshangjia/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussshitangshangjia',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussshitangshangjia',that.detail)
							setTimeout(()=>{
